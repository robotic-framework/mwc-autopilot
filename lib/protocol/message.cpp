...
void msg_sim_imu::decode(uint8_t *buf, uint8_t length) {
    if (length < 28) return;
...
uint8_t msg_sim_imu::encode(uint8_t *buf, uint8_t maxLength) {
...
void msg_sim_acc::decode(uint8_t *buf, uint8_t length) {
    if (length < 6) return;
    read((uint8_t *) acc, buf, 0, 6);
}

uint8_t msg_sim_acc::encode(uint8_t *buf, uint8_t maxLength) {
    return read(buf, (uint8_t *) acc, 0, 6);
}

void msg_sim_gyro::decode(uint8_t *buf, uint8_t length) {
    if (length < 6) return;
    read((uint8_t *) gyro, buf, 0, 6);
}

uint8_t msg_sim_gyro::encode(uint8_t *buf, uint8_t maxLength) {
